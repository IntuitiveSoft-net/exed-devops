...
Save your instance ID as environement variable as you are going to reuse it later on
...
The Instance ID will be saved to environement variable called `INSTANCE_ID`
...
## Task 12. Clean your lab environement
...
for route_table in $(aws ec2 describe-route-tables --filters "Name=vpc-id,Values=${VPC_ID}" "Name=association.main,Values=false" --query "RouteTables[].RouteTableId" --output text); do
...
for sg in $(aws ec2 describe-security-groups --filters Name=vpc-id,Values="${VPC_ID}" --query "SecurityGroups[?GroupName!='default'].GroupId" | jq -r '.[].GroupId'); do
  echo -e "\tDelete Security Group ${sg}"
  aws ec2 delete-security-group --group-id $sg
done

# Finally Delete you VPC
...
Cleaning VPC : vpc-006ddcf026ca43388
  ...
  Delete Subnet : net-rf0330303030d4
  Delete Subnet : net-gh9diddfbc9244
  Delete Security Group sg-0eaf3b02b8886a626
Delete VPC vpc-006ddcf026ca43388
```
