...
For the purpose of this training we propose a simplified standalone folder structure that can be easily refactored and enhanced to accommodate Ansible packaging into roles or collections.

```
ansible-project/            # This hierarchy represents an Ansible project
    tasks/                  # 
        foo-tasks.yml       # Generic tasks for playbooks
        bar-tasks.yml      
    templates/              #  Files for use with the template resource
        foo-bar.conf.j2     #  Templates end with .j2
    files/                  #
        foo.txt             #  Files for use with the copy resource
        bar.sh              #  Script files for use with the script resource
    vars/              
        a-var-file.yml      #  Variables file for playbooks
    library/                # folder for custom modules
        foo-bar/...
    foo.yml                 # A playbook
    bar.yml                 # Another playbook
    inventory.ini           # Inventory file 
```

An ansible boilerplate is available on your VM copy it into a new project :

```bash
cp -R ~/content/ansible-project ansible-nginx
```
...
Inside your `ansible-project` folder edit the `inventory.ini` file with the following content to connect to your nginx server. **Update the ansible_host variable with your EC2 instance VM IP address and update the path to your SSH key**
...
[webservers:vars]
ansible_ssh_private_key_file=~/.ssh/my-private-key.pem ansible_user=ubuntu
```
...
## Task. 11 Define playbook variables
...
## Task 12. Use variables in playbooks
...
## Task 13. Create a virtual host Jinja2 template Nginx configuration file
...
- name: Configure a new virtualhost          
  ...
  - name: copy nginx site.conf
    ansible.builtin.template:
      ...
      dest: /etc/nginx/sites-enabled/devops
      owner: root
      group: root
      mode: '0644'
    notify: restart nginx
...
Verify that you can no longer access your website using IP address only

![](images/vhost-result.png)

Instead open a browser page to your VM DNS entry : http://USER-NAME.devops.intuitivesoft.cloud
